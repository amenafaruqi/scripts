...
dumpfiles = np.arange(411,1901)     # For efficiency, only run after disc has stabilised
density_files = np.arange(400,1901)
...
def make_ODMs_directly(files, odms_dir, wavelength=0.8):
    for i, file in enumerate(files):
        if i > 0:
            prev_file_no = str(files[i-1]).rjust(5, '0')
            print(f'-------------------- MOVING FITS FILE NUMBER {prev_file_no} --------------------')
        ...
        print(f'============== Progress: {i}/{len(files)} ==================') 
        file_no = str(files[i]).rjust(5, '0')
        filename = f'{sim}/{file_prefix}_{file_no}'
        delete_old_data(old_dirs)      # delete previous run FITS files
        call(['mcfost', parafile, '-phantom', filename, '-od'])
        call(['mcfost', parafile, '-phantom', filename, '-od', '-img', str(wavelength)])
...
for dust_mass in dust_masses:
    ...
    odms_dir = f'./odms_method2/odms_{str(wavelength)}um/odms_{sim}_{str(dust_mass)}'
    ...
    make_ODMs_directly(dumpfiles, odms_dir, wavelength=wavelength)
